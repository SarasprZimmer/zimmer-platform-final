...
export default function PasswordChangeModal({ 
  ...
  const sendVerificationCode = async () => {
    ...
    setMsg(null);
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:8000"}/api/auth/send-password-reset-code`, {
        method: "POST",
        headers: { 
  ...
  const submitPasswordChange = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:8000"}/api/user/password`, {
        method: "POST",
        headers: { 
          "Content-Type": "application/json",
          "Authorization": `Bearer ${localStorage.getItem("access_token")}`
